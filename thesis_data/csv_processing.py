    ...
    try:
        ...
        for key in udp_data:
            for i in range(udp_data[key]['data_stream'].shape[0]):
                ...
                #discord_help[4] = udp_data[key]['loss']
                #discord_help[4] = tcp_data[key]['retransmission'].to_numpy()[i]
                discord_help[4] = TimeSinceLastRetran[key][i]
                discord_help[5] = tcp_data[key]['tcp_len'].to_numpy()[i]
        ...
        discord.columns = ['voice', 'TSLP', 'ctrl', 'TSLB', 'TSLR', 'tcp_len', 'max_jitter', 'avg_jitter', 'avg_missing_packets', 'quality']
